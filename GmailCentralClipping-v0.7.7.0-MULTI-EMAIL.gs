...
function doGet(e) {
  try {
    ...
    if (action === 'health') {
      return json_({ok:true,version:'0.7.7.0',mode:'gmail-multi-email-consolidation-all-candidates',mailbox:safeMailbox_()});
    }
...
function mattersFeed_(requestedDate) {
  ...
  // v0.7.7.0: NÃO usamos mais um subject:"..." único na busca do Gmail.
  ...
  return json_({
    ok:true,version:'0.7.7.0',date:target.dash,mailbox:safeMailbox_(),
...
function isCoverMonitorSubject_(subject){
  ...
  return /^MONITORAMENTO CAPAS? DE JORNAIS(?: [0-9]+)?$/.test(s);
}
