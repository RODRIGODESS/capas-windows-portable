...
function doGet(e) {
  try {
    ...
    if (action === 'health') {
      return json_({ok:true,version:'0.7.5.3',mode:'gmail-up-to-5-pages-cover-ranking-nyt-fix',mailbox:safeMailbox_()});
    }
...
function mattersFeed_(requestedDate) {
  ...
  Object.keys(VEHICLES).forEach(function(name){
    ...
    // v0.7.5.0: o APK pode receber até as 5 primeiras páginas de cada jornal.
    // Enviamos até 5 links EXATOS do mesmo veículo para que o app compare todas
    // as páginas e escolha automaticamente a candidata com maior confiança de capa.
    unique.slice(0,5).forEach(function(x,index){
      matters.push({name:name,matterUrl:x.url,vehicle:x.vehicle,priority:x.priority,candidateIndex:index+1});
    });
  });

  return json_({
    ok:true,version:'0.7.5.3',date:target.dash,mailbox:safeMailbox_(),
