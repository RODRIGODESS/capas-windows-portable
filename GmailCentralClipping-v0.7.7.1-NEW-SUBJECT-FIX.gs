...
function doGet(e) {
  try {
    ...
    if (action === 'health') {
      return json_({ok:true,version:'0.7.7.1',mode:'gmail-multi-email-consolidation-all-candidates',mailbox:safeMailbox_()});
    }
...
function mattersFeed_(requestedDate) {
  ...
  return json_({
    ok:true,version:'0.7.7.1',date:target.dash,mailbox:safeMailbox_(),
